...
namespace Golay
{
    ...
    public static class GolayCodec
    {
        ...
        public static byte[] Encode(byte[] source, out int length)
        {
            ...
            //if (finalLength != codedBytes) throw new Exception($"Miscalculated final size: expected {finalLength}, got {codedBytes} ({(double)codedBytes/(source.Length + padding)} times source)");
        ...
        public static byte[] Decode(byte[] encoded, out int totalErrors)
        {
            ...
            while (codeBytes < encoded.Length)
            {
                ...
                /*if (errs != 0)
                {
                    throw new Exception($"{codeWord:X6} -> {dataWord:X6}");
                }*/
        ...
        /// input should be 12 data-bits
        /// return data is 11 check-bits plus 12 data bits.
        /// </summary>
        private static uint Golay(uint orig)
        {
            var codeWord = orig & 0xfff; // lower 12 bits only
        ...
        {
            byte w; // current syndrome limit weight: 2 or 3.
            uint mask; // mask for bit flipping
            int j; // indices
            uint i; // indices
            uint s; // calculated syndrome
            ...
            while (j < 23) // flip each trial bit
            {
                ...
                {
                    if (j > 0) // restore prev trial bit
                    {
                        cw = orig | mask;
                        mask += mask; // move to next bit
                    }
                    else
                    {
                        cw = orig ^ mask; // flip next trial bit
                        w = 2; // lower threshold during this phase
                    }
                }
